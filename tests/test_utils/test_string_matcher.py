...
class TestStringMatcher(unittest.TestCase):
    def setUp(self):
        StringMatcher._cache["valid_regex_patterns"] = set()

    def test_raise_for_invalid_regex_patterns_with_no_cache_usage_and_invalid_patterns(
        self,
    ) -> None:
        ...
        self.assertEqual(len(StringMatcher._cache["valid_regex_patterns"]), 0)
        self.assertTrue(
            tuple(invalid_patterns) not in StringMatcher._cache["valid_regex_patterns"]
        )

    def test_raise_for_invalid_regex_patterns_with_no_cache_usage_and_valid_regex_patterns(
        self,
    ) -> None:
        valid_patterns = {".git", ".venv"}
        StringMatcher._raise_for_invalid_regex_patterns(valid_patterns)
        self.assertEqual(len(StringMatcher._cache["valid_regex_patterns"]), 1)
        self.assertTrue(
            tuple(valid_patterns) in StringMatcher._cache["valid_regex_patterns"]
        )

    def test_raise_for_invalid_regex_patterns_with_cache_usage_and_invalid_patterns(
        self,
    ) -> None:
        invalid_patterns = {"*.", ".venv"}
        with self.assertRaises(ValueError) as context_manager:
    ...
    def test_raise_for_invalid_regex_patterns_with_cache_usage_and_valid_patterns(
        self,
    ) -> None:
        valid_patterns = {".git", ".venv"}
        for _ in range(3):
    ...
    def test_matches_any_regex_pattern_with_no_matching_patterns(self) -> None:
        ...
        self.assertFalse(StringMatcher.matches_any_regex_pattern(string, patterns))
        self.assertEqual(len(StringMatcher._cache["valid_regex_patterns"]), 1)
        self.assertTrue(tuple(patterns) in StringMatcher._cache["valid_regex_patterns"])
